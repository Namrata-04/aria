...
class StorageManager:
    """Hybrid storage manager that can use MongoDB or file storage"""
    ...
    async def initialize(self):
        ...
        if self.use_mongodb and self.mongo_service is not None:
            try:
                connected = await self.mongo_service['connect']()
                if connected is False:
                    print("⚠️ MongoDB connection failed, falling back to file storage")
                    self.use_mongodb = False
    ...
    async def create_session(self, session_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.use_mongodb and self.mongo_service is not None:
            return await self.mongo_service['create_session'](session_id, user_id)
        else:
            return await self.file_service['create_session'](session_id, user_id)
    
    async def get_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        """Get session by ID"""
        if self.use_mongodb and self.mongo_service is not None:
            return await self.mongo_service['get_session'](session_id)
        else:
            return await self.file_service['get_session'](session_id)
    ...
    async def get_search_history(self, session_id: str) -> List[Dict[str, Any]]:
        """Get search history for session"""
        if self.use_mongodb and self.mongo_service is not None:
            return await self.mongo_service['get_search_history'](session_id)
        else:
            return await self.file_service['get_search_history'](session_id)
    ...
    async def get_saved_research(self, session_id: str) -> List[Dict[str, Any]]:
        """Get saved research for session"""
        if self.use_mongodb and self.mongo_service is not None:
            return await self.mongo_service['get_saved_research'](session_id)
        else:
            return await self.file_service['get_saved_research'](session_id)
    ...
    def get_storage_type(self) -> str:
        ...
        return "MongoDB" if self.use_mongodb else "File Storage"
